...
from core.events import create_event_context, event_emitter
...
def run_agent(agent_id: str, repos: Repositories, *, max_turns: int = MAX_TURNS) -> dict[str, Any]:
    ...
    from core.model_config import get_model_definition
    from core.pruning import needs_pruning, prune_conversation
    ...
    for _ in range(max_turns):
        ...
        if needs_pruning(items, agent.task, model_def.context_window, model_def.pruning.threshold):
            ...
            if (
                ...
                if session:
                    from core.summarization import generate_summary
